...
public abstract class RecordBuilderBase<T extends IndexedRecord> implements RecordBuilder<T> {
  ...
  /**
   * Gets the default value of the given field, if any. Pass in a conversion to
   * convert data to logical type class. Please make sure the schema does have a
   * logical type, otherwise an exception would be thrown out.
   * 
   * @param field      the field whose default value should be retrieved.
  ...
  protected Object defaultValue(Field field, Conversion<?> conversion) throws IOException {
    ...
    if (conversion == null || logicalType == null) {
      ...
      return Conversions.convertToLogicalType(rawDefaultValue, schema, logicalType, conversion);
    }
  }
